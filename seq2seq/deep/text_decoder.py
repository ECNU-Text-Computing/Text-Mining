...
class Decoder(BaseModel):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_classes,
        ...
        super(Decoder, self).__init__(vocab_size, embed_dim, hidden_dim, num_classes,
                                      dropout_rate, learning_rate, num_epochs, batch_size,
                                      criterion_name, optimizer_name, gpu, **kwargs)
        # 基本参数设置
        # encoder layer, decoder layer堆叠层数
        self.num_layers = 6
        ...
        # 包含self-attention和feed forward的decoder层
        decoder_layer = nn.TransformerDecoderLayer(self.embed_dim, self.num_heads, self.hidden_dim, self.dropout_rate,
                                                   device=self.device, batch_first=True)
        # decoder层堆叠
    ...
    def forward(self, x):
        ...
        encode = content * math.sqrt(self.embed_dim)  # [batch_size, seq_len, embed_dim]
        encode = self.pos_encoder(encode)  # [batch_size, seq_len, embed_dim]
        encode = self.transformer_encoder(encode)  # [batch_size, seq_len, embed_dim]
        output = self.transformer_decoder(content, encode)  # [batch_size, seq_len, embed_dim]
...
if __name__ == '__main__':
    ...
    if args.phase == 'test':
        ...
        model = Decoder(vocab_size, embed_dim, hidden_dim, num_classes,
                        dropout_rate, learning_rate, num_epochs, batch_size,
