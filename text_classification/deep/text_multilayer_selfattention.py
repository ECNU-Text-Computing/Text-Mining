...
# Text MultiLayerSelfAttention: input -> embedding -> 特征乘以三个矩阵得到Q K V ->Q 和K 相乘得到注意力矩阵A并归一化
class MultiLayerSelfAttention(BaseModel):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_classes,
                 dropout_rate, learning_rate, num_epochs, batch_size,
                 criterion_name, optimizer_name, gpu, **kwargs):
        super(MultiLayerSelfAttention, self).__init__(vocab_size, embed_dim, hidden_dim, num_classes,
                                                      dropout_rate, learning_rate, num_epochs, batch_size,
                                                      criterion_name, optimizer_name, gpu, **kwargs)
        # 基本参数设置
        # 堆叠层数
        ...
        self.q2 = nn.Linear(self.dim_k, self.dim_k)
        self.k2 = nn.Linear(self.dim_k, self.dim_k)
        self.v2 = nn.Linear(self.dim_v, self.dim_v)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            att = nn.Softmax(dim=-1)(torch.bmm(Q, K.permute(0, 2, 1))) * self._norm_fact
            # Q * K.T() * V
            out = torch.bmm(att, V)  # [batch_size, seq_len, dim_v]
            out = torch.sum(out, 1)  # [batch_size, dim_v]
            out_list.append(out)
        out_final = torch.stack(out_list, dim=0)
        print(out_final.size())
        out = out_final.mean(dim=0)
        print(out.size())
        out = F.relu(out)
...
if __name__ == '__main__':
    ...
    if args.phase == 'test':
        ...
        model = MultiLayerSelfAttention(vocab_size, embed_dim, hidden_dim, num_classes,
                                        dropout_rate, learning_rate, num_epochs, batch_size,
    ...
    print('Done MultiLayerSelfAttention!')
